...
src_prepare() {
	...
	# The ImGui source override bypasses FetchContent's PATCH_COMMAND, so apply
	# the exact upstream patch here.
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DNON_PORTABLE=ON
		# Gentoo's cmake.eclass seeds BUILD_SHARED_LIBS=ON, while upstream SoH
		# explicitly intends its remaining private helper libraries to be static.
